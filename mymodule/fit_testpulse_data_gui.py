...
def parse_optional_float(value: str | float | int | None) -> float:
    if value is None:
        return math.nan
...
def parse_optional_int(value: str | int | None) -> int:
    if value is None or str(value).strip() == "":
        return 0
    return int(float(str(value)))


def parse_optional_bool(value: str | int | bool | None) -> bool | None:
    if value is None:
        return None
...
def row_has_fit_result(row: dict[str, str]) -> bool:
    return any((row.get(name) or "").strip() for name in (
        "fit_passed", "peak_adu", "sigma_adu", "amplitude", "baseline",
        "n_events", "n_fit", "chi2_ndf", "fit_min_adu", "fit_max_adu", "reason",
    ))


def result_from_row(row: dict[str, str], cfg: tp.FitConfig) -> tp.FitResult | None:
    if not row_has_fit_result(row):
        return None
    time_id = row["time_id"]
    ...
    return tp.FitResult(
        ...
        file_path=file_path,
        fec=int(row["fec"]),
        alive=bool(parse_optional_bool(row.get("fit_passed"))),
        peak_adu=parse_optional_float(row.get("peak_adu")),
        peak_adu_err=parse_optional_float(row.get("peak_adu_err")),
        sigma_adu=parse_optional_float(row.get("sigma_adu")),
        sigma_adu_err=parse_optional_float(row.get("sigma_adu_err")),
        amplitude=parse_optional_float(row.get("amplitude")),
        baseline=parse_optional_float(row.get("baseline")),
        n_events=parse_optional_int(row.get("n_events")),
        n_fit=parse_optional_int(row.get("n_fit")),
        chi2_ndf=parse_optional_float(row.get("chi2_ndf")),
        fit_min_adu=parse_optional_float(row.get("fit_min_adu")),
        fit_max_adu=parse_optional_float(row.get("fit_max_adu")),
...
def load_review_states(cfg: tp.FitConfig) -> dict[tuple[str, int], ReviewState]:
    ...
    with cfg.review_csv.open(newline="") as f:
        ...
        for row in reader:
            time_id = row["time_id"]
            fec = int(row["fec"])
            states[(time_id, fec)] = ReviewState(
                peak_ok=parse_optional_bool(row.get("peak_ok")),
                use_fit=parse_optional_bool(row.get("use_fit")),
                result=result_from_row(row, cfg),
            )
    return states


def review_row(cfg: tp.FitConfig,
               time_id: str,
               fec: int,
    ...
    return [
        ...
        fec,
        "" if state.peak_ok is None else int(state.peak_ok),
        "" if state.use_fit is None else int(state.use_fit),
        "" if result is None else int(result.alive),
        "" if result is None else result.peak_adu,
        "" if result is None else result.peak_adu_err,
        "" if result is None else result.sigma_adu,
        "" if result is None else result.sigma_adu_err,
        "" if result is None else result.amplitude,
        "" if result is None else result.baseline,
        "" if result is None else result.n_events,
        "" if result is None else result.n_fit,
        "" if result is None else result.chi2_ndf,
        "" if result is None else result.fit_min_adu,
        "" if result is None else result.fit_max_adu,
        "" if result is None else result.reason,
        file_path,
    ]
...
def save_review_states(cfg: tp.FitConfig,
    ...
    with cfg.review_csv.open("w", newline="") as f:
        ...
        for time_id, fec in all_items:
            writer.writerow(review_row(cfg, time_id, fec, states.get((time_id, fec), ReviewState())))


def write_summary_from_review(cfg: tp.FitConfig,
                              all_items: list[tuple[str, int]],
                              states: dict[tuple[str, int], ReviewState]) -> None:
    results: list[tp.FitResult] = []
    manual_selection: dict[tuple[str, int], bool] = {}
    for time_id, fec in all_items:
        state = states.get((time_id, fec), ReviewState())
        manual_selection[(time_id, fec)] = bool(
            state.peak_ok is True
            and state.result is not None
            and np.isfinite(state.result.peak_adu)
        )
...
class TestPulseReviewTkGUI:
    ...
    def on_fit(self) -> None:
        ...
        for fec in active:
            ...
            state.use_fit = bool(np.isfinite(state.result.peak_adu))
            if state.result is not None and np.isfinite(state.result.peak_adu):
    ...
    def on_save_summary(self) -> None:
        ...
        if missing:
            ...
            self.update_status()
            return
        invalid = [fec for fec in self.cfg.fec_ids
                   if self.state_for(time_id, fec).peak_ok is True
                   and self.state_for(time_id, fec).result is not None
                   and not np.isfinite(self.state_for(time_id, fec).result.peak_adu)]
        if invalid:
            self.current_state().message = "Save blocked: invalid peak for " + ", ".join(f"FEC{fec}" for fec in invalid)
            if self.save_button is not None:
                self.save_button.configure(fg_color="#dc2626", hover_color="#b91c1c")
            self.update_status()
            return
